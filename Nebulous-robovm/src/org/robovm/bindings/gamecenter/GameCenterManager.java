...
public class GameCenterManager {
	 ...
	 * 
	 * @param keyWindow
	 *            KeyWindow can't be accessed from the Delegates sometimes, so
	 *            we need to save a reference
	 * @param listener
	 */
	...
	public void login() {
		...
		if (getIosVersion() >= IOS_6) {
			GKLocalPlayer.getLocalPlayer().setAuthenticateHandler(
					new VoidUIViewControllerNSErrorBlock() {
						@Override
						public void invoke(UIViewController viewController,
								NSError error) {
							// If the device does not have an authenticated
							// player, show the login dialog
							if (viewController != null) {
								keyWindow.getRootViewController()
										.presentViewController(viewController,
												true, null);
							}
							// If the viewController is null and the player is
							// authenticated, the login is completed
							else if (GKLocalPlayer.getLocalPlayer()
									.isAuthenticated()) {
								listener.playerLoginCompleted();
							}
							// If the viewController is null and the player is
							// not authenticated the login has failed
							else {
			...
			GKLocalPlayer.getLocalPlayer().authenticateWithCompletionHandler(
					new VoidNSErrorBlock() {
						@Override
						public void invoke(NSError error) {
							if (GKLocalPlayer.getLocalPlayer()
									.isAuthenticated()) {
								listener.playerLoginCompleted();
							} else {
								listener.playerLoginFailed();
	...
	/**
	 * Reset the achievements progress for the local player. All the entries for
	 * the local player are removed from the server.
	 */
	...
	/**
	 * Load all the Leaderboards for the Game. Warning: If using iOS5 or less
	 * the Leaderboard object will only include the Category (identifier)
	 */
				...
				public void invoke(NSArray array, NSArray array2, NSError error) {
					if (error != null) {
						...
						for (NSObject category : (NSArray<NSObject>) array) {
							...
							leaderboard.setCategory(((NSString) category)
									.toString());
	...
	/**
	 * Return the id of a leaderboard (category or identifier, depending on iOS
	 * version)
	 * 
	 * @param leaderboard
	 * @return
	...
	public void showAchievementsView() {
		...
		if (getIosVersion() >= IOS_6) {
			...
			gameCenterView
					.setGameCenterDelegate(new GKGameCenterControllerDelegate.Adapter() {
						@Override
						public void gameCenterViewControllerDidFinish(
								GKGameCenterViewController gameCenterViewController) {
							gameCenterViewController.dismissViewController(
									true, null);
						}
					});
			gameCenterView
					.setViewState(GKGameCenterViewControllerState.GKGameCenterViewControllerStateAchievements);
			keyWindow.getRootViewController().presentViewController(
					gameCenterView, true, null);
		} else { // If iOS version is 6 or less we use the deprecated method
			GKAchievementViewController gameCenterView = new GKAchievementViewController();
			gameCenterView
					.setAchievementDelegate(new GKAchievementViewControllerDelegate.Adapter() {
						@Override
	...
	public void showLeaderboardsView() {
		...
		if (getIosVersion() >= IOS_6) {
			...
			gameCenterView
					.setViewState(GKGameCenterViewControllerState.GKGameCenterViewControllerStateLeaderboards);
			// gameCenterView
			// .setLeaderboardTimeScope(GKLeaderboardTimeScope.GKLeaderboardTimeScopeAllTime);
			gameCenterView
					.setLeaderboardIdentifier("NebulousHighScoreLeaderboard");
			keyWindow.getRootViewController().presentViewController(
					gameCenterView, true, null);
		} else { // If iOS version is 6 or less we use the deprecated method
			GKLeaderboardViewController gameCenterView = new GKLeaderboardViewController();
			gameCenterView
					.setTimeScope(GKLeaderboardTimeScope.GKLeaderboardTimeScopeAllTime);
			gameCenterView
					.setLeaderboardDelegate(new GKLeaderboardViewControllerDelegate.Adapter() {
						@Override
